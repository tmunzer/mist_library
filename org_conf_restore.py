...
def _common_restore(level, level_id, object_name, data):
...
def _wlan_restore(level, level_id, data, old_org_id, old_site_id):
    ...
    ids = _common_restore(level, level_id, 'wlans', data)
    old_wlan_id = next(iter(ids))
    new_wlan_id = ids[old_wlan_id]
    _wlan_restore_portal(level_id, old_org_id, old_site_id, old_wlan_id, new_wlan_id)
    wlan_id_dict.update(ids)

def _wlan_restore_portal(level_id, old_org_id, old_site_id, old_wlan_id, new_wlan_id):  
        if old_site_id == None:
...
def _restore_org(org_id, org):
    ...
    for data in org["wxrules"]:
        ...
        _common_restore('orgs',  org_id, 'wxrules', data)

    for data in org["wxtunnels"]:
        ids = _common_restore('orgs', org_id, 'wxtunnels', data)
    ...
    for data in org["sites"]:
        ...
        ids = _common_restore('orgs',  org_id, 'sites', site)
        ...
        if "maps" in data:
            for sub_data in data["maps"]:
                ...
                ids = _common_restore('sites', new_site_id, 'maps', sub_data)
        ...
        if "assetfilters" in data:
            for sub_data in data["assetfilters"]:
                _common_restore('sites', new_site_id, 'assetfilters', sub_data)

        if "assets" in data:
            for sub_data in data["assets"]:
                _common_restore('sites', new_site_id, 'assets', sub_data)

        if "beacons" in data:
            for sub_data in data["beacons"]:
                sub_data["map_id"] = _replace_id(sub_data["map_id"], map_id_dict)
                _common_restore('sites', new_site_id, 'beacons', sub_data)

        if "psks" in data:
            for sub_data in data["psks"]:
                sub_data["site_id"] = new_site_id
                _common_restore('sites', new_site_id, 'psks', sub_data)

        if "rssizones" in data:
            for sub_data in data["rssizones"]:
                _common_restore('sites', new_site_id, 'rssizones', sub_data)

        if "vbeacons" in data:
            for sub_data in data["vbeacons"]:
                sub_data["map_id"] = _replace_id(sub_data["map_id"], map_id_dict)
                _common_restore('sites', new_site_id, 'vbeacons', sub_data)

        if "webhooks" in data:
            for sub_data in data["webhooks"]:
                _common_restore('sites', new_site_id, 'webhooks', sub_data)

        if "wxtunnels" in data:
            for sub_data in data["wxtunnels"]:
                ids = _common_restore('sites', new_site_id,'wxtunnels', sub_data)
                wxtunnel_id_dict.update(ids)

        if "zones" in data:
            for sub_data in data["zones"]:
                sub_data["map_id"] = _replace_id(sub_data["map_id"], map_id_dict)
                _common_restore('sites', new_site_id, 'zones', sub_data)
        
        if "wlans" in data:
            for sub_data in data["wlans"]:
                _wlan_restore('sites', new_site_id, sub_data, old_org_id, old_site_id)

        if "wxtags" in data:
            for sub_data in data["wxtags"]:
                if sub_data["match"] == "wlan_id":
                    _replace_id(sub_data["values"], wlan_id_dict)
                ids = _common_restore('sites', new_site_id, 'wxtags', sub_data)
                wxtags_id_dict.update(ids)
        ...
        if "wxrules" in data:
            for sub_data in data["wxrules"]:
                ...
                _common_restore('sites', new_site_id, 'wxrules', sub_data)
    ...
    for data in org["templates"]:
        ...
        ids = _common_restore('orgs', org_id, 'templates', data)
        template_id_dict.update(ids)

    for data in org["wlans"]:
        _wlan_restore('orgs', org_id, data, old_org_id, None)

    for data in org["ssos"]: _common_restore('orgs', org_id, 'ssos', data)

    for data in org["ssoroles"]:
        cleaned_data = []
        for ssorole in data:
            cleaned_data.append(_clean_ssorole_privileges(ssorole))
        _common_restore('orgs', org_id, 'ssoroles', cleaned_data)    
...
def start_restore_org(mist_session, org_id, org_name, source_org_name):
    ...
    try:
        ...
        if backup:
            ...
            _restore_org(org_id, backup["org"])
            print()
