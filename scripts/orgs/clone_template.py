...
#####################################################################
#### SWITCH TEMPLATE ####
def _get_switch_template(src_session:mistapi.APISession, src_oid:str,src_template_id: str):
    try:
        message = f"Retrieving network template {src_template_id}"
        PB.log_message(message, display_pbar=False)
        resp = mistapi.api.v1.orgs.networktemplates.getOrgNetworkTemplate(src_session, src_oid, src_template_id)
...
def _deploy_switch_template( dst_session:mistapi.APISession, dst_oid:str, template:dict):
    try:
        message = f"Deploying network template {template.get('name')}"
        PB.log_message(message, display_pbar=False)
        resp = mistapi.api.v1.orgs.networktemplates.createOrgNetworkTemplate(dst_session, dst_oid, template)
        if resp.status_code == 200:
            PB.log_success(message, display_pbar=False)
        else:
            PB.log_failure(message, display_pbar=False)
...
def clone_switch_template(src_session:mistapi.APISession,dst_session:mistapi.APISession, src_oid:str, dst_oid:str,src_template_id: str, dst_template_name:str=None):
...
#####################################################################
#### WAN TEMPLATE ####
def _get_wan_template(src_session:mistapi.APISession, src_oid:str,src_template_id: str):
    try:
        message = f"Retrieving gateway template {src_template_id}"
        PB.log_message(message, display_pbar=False)
        resp = mistapi.api.v1.orgs.gatewaytemplates.getOrgGatewayTemplate(src_session, src_oid, src_template_id)
        if resp.status_code == 200:
            PB.log_success(message, display_pbar=False)
            return resp.data
        else:
...
def _deploy_wan_template( dst_session:mistapi.APISession, dst_oid:str, template:dict):
    try:
        ...
        PB.log_message(message, display_pbar=False)
        resp = mistapi.api.v1.orgs.gatewaytemplates.createOrgGatewayTemplate(dst_session, dst_oid, template)
        if resp.status_code == 200:
            PB.log_success(message, display_pbar=False)
        else:
            PB.log_failure(message, display_pbar=False)
...
########################        
#### HUB PROFILE ####
def _get_device_profile(src_session:mistapi.APISession, src_oid:str,src_template_id: str):
    try:
        message = f"Retrieving gateway template {src_template_id}"
        PB.log_message(message, display_pbar=False)
        resp = mistapi.api.v1.orgs.deviceprofiles.getOrgDeviceProfile(src_session, src_oid, src_template_id)
        if resp.status_code == 200:
            PB.log_success(message, display_pbar=False)
            return resp.data
        else:
...
def _deploy_device_profile( dst_session:mistapi.APISession, dst_oid:str, template:dict):
    try:
        message = f"Deploying gateway template {template.get('name')}"
        PB.log_message(message, display_pbar=False)
        resp = mistapi.api.v1.orgs.deviceprofiles.createOrgDeviceProfiles(dst_session, dst_oid, template)
        if resp.status_code == 200:
            PB.log_success(message, display_pbar=False)
        else:
            PB.log_failure(message, display_pbar=False)
...
def _get_wan_services(src_session: mistapi.APISession, src_oid:str):
    try:
        ...
        PB.log_message(message, display_pbar=False)
        resp = mistapi.api.v1.orgs.services.listOrgServices(src_session, src_oid, limit=1000)
        if resp.status_code == 200:
...
def _deploy_wan_services( dst_session:mistapi.APISession, dst_oid:str, service:dict):
    try:
        ...
        PB.log_message(message, display_pbar=False)
        resp = mistapi.api.v1.orgs.services.createOrgService(dst_session, dst_oid, service)
        if resp.status_code == 200:
            PB.log_success(message, display_pbar=False)
        else:
            PB.log_failure(message, display_pbar=False)
...
def _get_wan_servicepolicies(src_session: mistapi.APISession, src_oid:str):
    try:
        ...
        PB.log_message(message, display_pbar=False)
        resp = mistapi.api.v1.orgs.servicepolicies.listOrgServicePolicies(src_session, src_oid, limit=1000)
        if resp.status_code == 200:
            data = mistapi.get_all(src_session, resp)
            PB.log_success(message, display_pbar=False)
            return data
...
def _deploy_wan_servicepolicy( dst_session:mistapi.APISession, dst_oid:str, servicepolicy:dict, uuid_map:dict):
    try:
        src_tid = servicepolicy.get("id")
        message = f"Deploying gateway service policy {servicepolicy.get('name')}"
        PB.log_message(message, display_pbar=False)
        resp = mistapi.api.v1.orgs.servicepolicies.createOrgServicePolicy(dst_session, dst_oid, servicepolicy)
        if resp.status_code == 200:
...
def _get_wan_networks(src_session: mistapi.APISession, src_oid:str):
    try:
        ...
        PB.log_message(message, display_pbar=False)
        resp = mistapi.api.v1.orgs.networks.listOrgNetworks(src_session, src_oid, limit=1000)
        if resp.status_code == 200:
            data = mistapi.get_all(src_session, resp)
            PB.log_success(message, display_pbar=False)
            return data
...
def _deploy_wan_networks( dst_session:mistapi.APISession, dst_oid:str, network:dict):
    try:
        ...
        PB.log_message(message, display_pbar=False)
        resp = mistapi.api.v1.orgs.networks.createOrgNetwork(dst_session, dst_oid, network)
        if resp.status_code == 200:
            PB.log_success(message, display_pbar=False)
        else:
            PB.log_failure(message, display_pbar=False)
...
########################
def clone_wan_template(src_session:mistapi.APISession,dst_session:mistapi.APISession, src_oid:str, dst_oid:str,src_template_id: str, dst_template_name:str=None, deviceprofile:bool=False):
    uuid_map = {}
    network_names = []
    service_names = []
    servicepolicy_ids = []
...
def _select_template_type():
    while True:
        ...
        resp = input(f"Which type of template do you want to clone (0-{len(TEMPLATE_TYPES)-1})? ")
        try:
            resp_int = int(resp)
            return TEMPLATE_VALUES[resp_int]
        except:
            print(f"Invalid input. Only numbers between 0 and {len(TEMPLATE_TYPES)-1} are allowed")

def _retrieve_template_list(src_session:mistapi.APISession, src_oid:str, template_type:str):
    resp = None
    if template_type == "wlan":
        resp = mistapi.api.v1.orgs.templates.listOrgTemplates(src_session, src_oid, limit=1000)
    elif template_type == "lan":
        resp = mistapi.api.v1.orgs.networktemplates.listOrgNetworkTemplates(src_session, src_oid, limit=1000)
    elif template_type == "wan":
        resp = mistapi.api.v1.orgs.gatewaytemplates.listOrgGatewayTemplates(src_session, src_oid, limit=1000)
    elif template_type == "hub":
        resp = mistapi.api.v1.orgs.deviceprofiles.listOrgDeviceProfiles(src_session, src_oid, type="gateway", limit=1000)
    else:
...
def _select_template_to_clone(src_session:mistapi.APISession, src_oid:str, template_type:str):
    ...
    while True:
        ...
        try:
            ...
            print(f"Invalid input. Only numbers between 0 and {len(templates)-1} are allowed")



def start(src_apisession: mistapi.APISession, dst_apisession: mistapi.APISession=None, src_org_id:str=None, dst_org_id:str=None, dst_org_name=None, template_type:str=None, template_id:str=None, dst_template_name:str=None):
    '''
    ...
    elif template_type == "wlan":
        print("This feature is not yet available...")
        sys.exit(0)
    if not template_id:
        template_id = _select_template_to_clone(src_apisession, src_org_id, template_type)

    if src_org_id == dst_org_id and not dst_template_name:
        print("It is not possible to have multiple templates with the same name in a single org.")
    ...
    if template_type == "wlan":
        ...
        clone_switch_template(src_apisession, dst_apisession, src_org_id, dst_org_id, template_id, dst_template_name )
    elif template_type == "wan":
        clone_wan_template(src_apisession, dst_apisession, src_org_id, dst_org_id, template_id, dst_template_name )
    elif template_type == "hub":
        clone_wan_template(src_apisession, dst_apisession, src_org_id, dst_org_id, template_id, dst_template_name, True )
...
if __name__ == "__main__":
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hl:b:s:d:", [
                                "help", 
                                "src_oid=",
                                "dst_oid=",
                                "dst_org_name=",
