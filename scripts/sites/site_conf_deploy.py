...
def _deploy_wlan_portal(
    ...
    if SYS_EXIT:
        ...
        portal_image = (
            f"{FILE_PREFIX}_site_{old_site_id}_wlan_{old_wlan_id}.png"
        )
...
def _deploy_site_maps(
    ...
    image_name = (
        f"{FILE_PREFIX}_site_{old_site_id}_map_{old_map_id}.png"
    )
    LOGGER.debug("conf_deploy:_deploy_site_maps: image_name %s", image_name)
...
def _deploy_site(
    ...
    site_info = site_backup.get("site", {}).get("info", {})
    site_info["name"] = site_name
    old_site_id = site_info["id"]

    PB.log_title(f" Deploying Site {site_name} ".center(80, "_"), display_pbar=False)
        ...
        if not site_data.get(step_name):
            ...
            if step_name == "settings":
                ...
                for step_data in site_data.get(step_name, []):
                    ...
                    if object_to_deploy and create:
                        _deploy_site_maps(
                            apisession, "", old_site_id, new_site_id, step_data
                        )
...
def _deploy_org_objects(
    apisession: mistapi.APISession,
    org_id: str,
    site_backup: dict,
    merge_action: str,
) -> None:
    LOGGER.debug("conf_deploy:_deploy_org_objects - merge action: %s", merge_action)
    if SYS_EXIT:
        sys.exit(0)

    PB.log_title(" Deploying Org Level Objects ".center(80, "_"), display_pbar=False)

    for step_name, step in ORG_STEPS.items():
        if step_name == "sites":
            continue
        elif step_name == "sitegroups":
            step.load_existing_objects(apisession, org_id)
            for sitegroup in site_backup.get(step_name, []):
                create, object_to_deploy, _ = step.search_existing_object(
                    sitegroup, sitegroup[step.attr_name], merge_action
                )
                if object_to_deploy and "site_ids" in object_to_deploy:
                    del object_to_deploy["site_ids"]
                if object_to_deploy and create:
                    _common_deploy(
                        apisession,
                        step,
        ...
        elif not site_backup.get(step_name):
            LOGGER.debug("Org Level > %s: nothing to process", step_name)
        else:
            LOGGER.debug("Org Level > %s: processing", step_name)  
            data = site_backup.get(step_name, {})
            step.load_existing_objects(apisession, org_id)
            create, object_to_deploy, _ = step.search_existing_object(
            ...
            if step_name == "networktemplate":
                ...
                if object_to_deploy and object_to_deploy.get("radius_config", {}).get("acct_servers", []):
                    for server in object_to_deploy["radius_config"]["acct_servers"]:
                        if "id" in server:
                            del server["id"]
            if step_name == "sitetemplate":
                if object_to_deploy and object_to_deploy.get("appliedSites", []):
                    del object_to_deploy["appliedSites"]
            if object_to_deploy and create:
                _common_deploy(
                    apisession,
                    step,
...
def _select_backup_org() -> None:
    ...
    while folder is None:
        resp = input(
            f"From which backed up org do you want to deploy a site (0-{i - 1}, or q to quit)? "
        )
        if resp.lower() == "q":
            console.error("Interruption... Exiting...")
            LOGGER.error("Interruption... Exiting...")
...
def _select_backup_site():
    ...
    while folder is None:
        resp = input(
            f"Which backed up site do you want to deploy (0-{i - 1}, or q to quit)? "
        )
        if resp.lower() == "q":
            console.error("Interruption... Exiting...")
            LOGGER.error("Interruption... Exiting...")
...
def _check_site_exists(
    apisession: mistapi.APISession,
    org_id: str,
    site_name_to_create: str,
) -> tuple[str, str]:
    LOGGER.debug("conf_deploy:_check_site_exists")
    site_id = ""
    existing_sites = mistapi.api.v1.orgs.sites.listOrgSites(apisession, org_id).data
    try:
        site_id = next(
            item["id"] for item in existing_sites if item["name"] == site_name_to_create
        )
        while True:
            print()
            console.warning(
                f'Site "{site_name_to_create}" already exists in the destination org! '
            )
            response = input(
                "What do you want to do: (r)eplace, set a (n)ew name or (a)bort? "
            )
            if response.lower() == "a":
                console.warning("Interruption... Exiting...")
                sys.exit(0)
            elif response.lower() == "r":
                return site_name_to_create, site_id
            elif response.lower() == "n":
                site_name_to_create = input("Name of the site to create: ")
                return _check_site_exists(apisession, org_id, site_name_to_create)
    except Exception:
        pass
...
def conf_deploy(
    ...
    if backup_path:
        try:
            ...
            console.critical("Unable to access the backup path")
            LOGGER.error("Exception occurred", exc_info=True)
            sys.exit(1)
    else:
        _go_to_backup_folder()
    ...
    if backup:
        print()
        site_name_to_create = input("Name of the site to create: ")
        site_name_to_create, site_id = _check_site_exists(apisession, org_id, site_name_to_create)

        _display_warning(
            "Are you sure about this? Do you want to import the site configuration into the organization {0} with the id {1} (y/N)? ".format(
                org_name, org_id
            )
        )
        
        _deploy_org_objects(apisession, org_id, backup, merge_action)
        _deploy_site(apisession, org_id, site_name_to_create, site_id, backup, merge_action)
...
def start(
    ...
    conf_deploy(apisession, org_id, org_name, backup_path, merge_action)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Deploy site backup/template file.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
python3 ./site_conf_deploy.py
python3 ./site_conf_deploy.py --org_id=203d3d02-xxxx-xxxx-xxxx-76896a3330f4 -n "my test org"
        """,
    )
    ...
    parser.add_argument(
        "-f",
        "--backup_path",
        help="Path to the folder where to save the site backup",
    )
    # parser.add_argument(
    ...
    LOGGER.info("Backup folder path: %s", BACKUP_FOLDER_PATH)
    ...
    start(APISESSION, ORG_ID, ORG_NAME, BACKUP_FOLDER_PATH, merge_action=MERGE_ACTION)
