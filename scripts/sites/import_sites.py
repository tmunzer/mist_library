...
-s, --source=       Location of the source JSON file to use as template for the site creation. 
                    This file can be generated by exporting a site configuration from the Mist UI.
...
def _create_sites(
    ...
    src_site_data = {}
    if src_file:
        message = "Retrieving Sites data from source file"
        PB.log_message(message, display_pbar=False)
        try:
            src_site_data = json.load(open(src_file, "r", encoding="utf-8"))
            PB.log_success(message, display_pbar=False)
        except Exception:
            PB.log_failure(message, display_pbar=False)
            LOGGER.error("Exception occurred", exc_info=True)
    ...
    for site in sites:
        ...
        if not new_site:
            ...
            if source_site_id:
                ...
                PB.inc()
            _update_site(apisession, site, new_site["id"], src_site_data)
...
def start(
    ...
    :param  str                 src_file        Optional, path to the JSON file with the sites used as template
                                                This file can be generated by exporting a site configuration from the Mist UI.
    ...
    _create_sites(apisession, org_id, sites, parameters, src_file, geocoder)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--source", dest="source_file", default="", help="Location of the source JSON file to use as template for the site creation. This file can be generated by exporting a site configuration from the Mist UI.")
    ...
    if not CSV_FILE:
        ...
        start(APISESSION, CSV_FILE, SRC_FILE, ORG_ID, ORG_NAME, GOOGLE_API_KEY)
